...
class RoadNetworkMapFull:
    def __init__(self, dir, zone_range, unit_length):
        ...
        for line in wayFile.readlines():
            item_list = line.strip().split()
            roadId = int(item_list[0])
            wayId = int(item_list[2])
            # lanes_num = int(item_list[3])
            # is_oneway = int(item_list[4])
            self.roadid[roadId] = roadId
            self.wayType[roadId] = wayId
            # self.lanes[roadId] = lanes_num
            # self.oneway[roadId] = is_oneway

        self.long_num = int((self.calSpatialDistance(self.zone_range[0], self.zone_range[1], self.zone_range[0],    # 容纳最大网格数
                                                self.zone_range[3]) + unit_length - 1) / unit_length)
