...
class GatedFusion(nn.Module):
    def __init__(self, d_model, speed_flag):
        ...
        self.speed_fc = nn.Linear(self.hid_dim,self.hid_dim)

    def forward(self, HS, HT, HV):
        '''
        gated fusion
...
class GraphRefinementLayer(nn.Module):
    def __init__(self, gnn_type, d_model, num_heads, speed_flag, dropout=0.1):
        super().__init__()
        self.d_model = d_model
        self.norm_1 = GraphNorm(d_model)
        self.norm_2 = GraphNorm(d_model)
        self.attn = GatedFusion(d_model, speed_flag)
    ...
    def forward(self, y, g, speed, mask2d=None):
        """
        ...
        x = x + self.dropout_1(self.attn(x2, y, speed))
...
class Encoder(nn.Module):
    def __init__(self, gnn_type, d_model, N, speed_flag,  device, heads=8):
        super().__init__()
        self.N = N
        ...
        self.refines = nn.ModuleList([
            GraphRefinementLayer(gnn_type, d_model,  heads, speed_flag) for _ in range(N)
        ])
        self.norm = Norm(d_model)
        # self.speed_flag =flag


    def forward(self, src, speed_cons, g, mask3d=None, mask2d=None):
        """
        ...
        for i in range(self.N):
            ...
            x, g = self.refines[i](x, g, speed_cons, mask2d)
