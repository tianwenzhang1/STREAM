...
import osmium as o
from preprocess.const import candi_highway_types
        ...
        if 'highway' in w.tags and w.tags['highway'] in self.candi_highway_types:
            ...
            for n in w.nodes:
                if n.location.valid():
                    ...
                    print("Invalid location for node:", n.ref)

            lane_numbers = int(w.tags.get('lanes', '1'))
            is_oneway = 1 if w.tags.get('oneway', 'yes') == 'yes' else 0

            if 'oneway' in w.tags:
                edge_attr = {'eid': self.eid, 'coords': full_coords, 'raw_eid': raw_eid, 'highway': w.tags['highway'],
                             'lane_numbers': lane_numbers, 'is_oneway': is_oneway}
                ...
                edge_attr = {'eid': self.eid, 'coords': reversed_full_coords, 'raw_eid': raw_eid,
                             'highway': w.tags['highway'], 'lane_numbers': lane_numbers, 'is_oneway': is_oneway}
...
def store_osm(rn, target_path):
    ...
    for stcoord, encoord, data in rn.edges(data=True):
        edges[data['eid']] = {'st': node_map[stcoord],
                              ...
                              'coords': data["coords"],
                              'type': data['highway'],
                              'lane_numbers': data.get('lane_numbers', 1),
                              'is_oneway': data.get('is_oneway', 1)}
    ...
    with open(os.path.join(target_path, 'wayTypeOSM.txt'), 'w+') as waytype:
        for idx, k in enumerate(sorted(edges.keys())):
            waytype.write(f'{"%-6d"%idx} {"%-10s"%edges[k]["type"]} {"%-4d"%candi_highway_types[edges[k]["type"]]} {edges[k]["lane_numbers"]} {edges[k]["is_oneway"]}\n')
        waytype.close()

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--input_path', default = './data/Porto.osm.pbf', help='the input path of the original osm data')
    parser.add_argument('--output_path', default = './data/Porto_roadnet2', help='the output directory of the constructed road network')
